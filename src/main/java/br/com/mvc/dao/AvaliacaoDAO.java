...
public class AvaliacaoDAO extends MysqlDAO {
    ...
    public Avaliacao buscarPorId(Long id) {
        String sql = "SELECT a.*, f.titulo titulo_filme, u.nome nome_usuario FROM avaliacoes a " +
                "INNER JOIN filmes f ON f.id=a.filme_id INNER JOIN usuarios u ON u.id=a.usuario_id " +
                "WHERE a.id=?";
        List<Avaliacao> lista = consultar(sql, id);
    ...
    public Avaliacao buscarPorIdEUsuario(Long id, Long usuarioId) {
        ...
        List<Avaliacao> lista = consultar(sql, id, usuarioId);
        return lista.isEmpty() ? null : lista.get(0);
    }
    ...
    public void salvar(Avaliacao a) {
        ...
        try {
            super.executarUpdate(sql, a.getFilmeId(), a.getUsuarioId(), a.getNota(), a.getComentario());
    ...
    public void alterar(Avaliacao a) {
        String sql = "UPDATE avaliacoes SET nota=?, comentario=?, data_avaliacao=CURRENT_TIMESTAMP " +
                "WHERE id=? AND usuario_id=?";
        try {
            int alterados = super.executarUpdate(
                    sql,
                    a.getNota(),
                    a.getComentario(),
                    a.getId(),
    ...
    public void deletar(Long id, Long usuarioId) {
        String sql = "DELETE FROM avaliacoes WHERE id=? AND usuario_id=?";
        try {
            int removidos = super.executarUpdate(sql, id, usuarioId);
    ...
    private List<Avaliacao> consultar(String sql, Object... parametros) {
        ...
        try (ResultSet rs = super.executar(sql, parametros)) {
            while (rs.next()) {
                ...
                a.setFilmeId(rs.getLong("filme_id"));
                ...
                a.setTituloFilme(rs.getString("titulo_filme"));
                a.setNomeUsuario(rs.getString("nome_usuario"));
                lista.add(a);
            }
        } catch (SQLException e) {
            throw new RuntimeException("Erro ao consultar avaliações.", e);
        }
