...
public class RecomendacaoServlet extends BaseServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String[] values = req.getParameterValues("categoria");
        if (values != null)
            for (String v : values)
                try {
                    categorias.add(Long.valueOf(v));
                } catch (Exception ignored) {
                }
        req.setAttribute("categorias", categoriaService.listar());
        req.setAttribute("selecionadas", categorias);
        req.setAttribute("filmes", service.recomendar(categorias));
